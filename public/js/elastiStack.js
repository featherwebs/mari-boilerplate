...
;( function( window ) {
	...
	ElastiStack.prototype._setStackStyle = function() {
		...
		if( item2 ) {
			...
			setTransformStyle( item2, is3d ? 'translate3d(0,0,0)' : 'translate(0,0)' );
		}

		if( item3 ) {
			item3.style.opacity = 1;
			item3.style.zIndex = 2;
			setTransformStyle( item3, is3d ? 'translate3d(0,0,0)' : 'translate(0,0)' );
		}
	};
	...
	ElastiStack.prototype._moveAway = function( instance ) {
		...
		var self = this,
			onEndTransFn = function() {
				...
				// reset first item
				setTransformStyle( instance.element, is3d ? 'translate3d(0,0,0px)' : 'translate(0,0,0)' );
	...
	ElastiStack.prototype._onDragMove = function( instance, event, pointer ) {
		if( this._outOfBounds( instance ) ) {
			...
			var item2 = this._secondItem(), item3 = this._thirdItem();
			if( item2 ) {
				setTransformStyle( item2, is3d ? 'translate3d(' + ( instance.position.x * .6 ) + 'px,' + ( instance.position.y * .6 ) + 'px, 0px)' : 'translate(' + ( instance.position.x * .6 ) + 'px,' + ( instance.position.y * .6 ) + 'px)' );
			}
			if( item3 ) {
				setTransformStyle( item3, is3d ? 'translate3d(' + ( instance.position.x * .3 ) + 'px,' + ( instance.position.y * .3 ) + 'px, 0px)' : 'translate(' + ( instance.position.x * .3 ) + 'px,' + ( instance.position.y * .3 ) + 'px)' );
			}
		}
	};
	...
	ElastiStack.prototype.next = function( instance, event, pointer ) {
		this._moveAway( this.draggie );
	};
